...
const text1 = document.querySelector('.show');

btn1.addEventListener('click',button);
function button(){
    text1.classList.toggle('hide');
    btn1.textContent=text1.classList.contains('hide')?('Read More'):('Read Less');
}


const btn2 = document.querySelector('#btn-1');
const text2 = document.querySelector('.show-1');
btn2.addEventListener('click',display);
function display(){
    text2.classList.toggle('hide-1');
    btn2.textContent=text2.classList.contains('hide-1')?('Read More'):('Read Less');
}
